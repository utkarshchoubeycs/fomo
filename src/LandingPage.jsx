...
function LandingPage2() {
    ...
    const SCROLL_INCREMENT = 0.1; // Amount to increment/decrement on each wheel event
    ...
    return (
        <div onWheel={handleWheel} style={{background:getBackgroundForStage(stage), height: '100vh', width: '100vw', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            {
                ringData2[stage].rings.map(ringMetadata => <RingComponent size={ringMetadata.size} borderColor={ringMetadata.borderColor} stage={stage} />)
            }
        </div>
    );
}
